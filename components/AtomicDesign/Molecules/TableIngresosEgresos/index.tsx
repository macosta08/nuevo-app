...
import { floatToCurrencyString, isoStringToDate } from 'utils/functions/currencyString';
...
function TableIngresosEgresos({ data }: { data: IngresosEgresosTotalProps[] }) {
  ...
  const headers = ['Concepto', 'Monto', 'Fecha', 'Usuario', 'Descripción', 'Acciones'];
          ...
          <TableBody>
            {data && data?.map((item, index) => (
              ...
              >
                ...
                <TableCell className='table-cell'>{floatToCurrencyString(item?.monto)}</TableCell>
                <TableCell className='table-cell'>{isoStringToDate(item?.fecha)}</TableCell>
                <TableCell className='table-cell'>{item?.user?.name}</TableCell>
                <TableCell className='table-cell truncate'>{item?.descripcion}</TableCell>
                <TableCell className='table-cell'> 
                  <Button onClick={() => router.push(`/ingresos-egresos/${item?.id}`)}>
                    Editar
                  </Button>
                </TableCell>
              </TableRow>
